...
def load_dicom(folder, skip_keywords=None):
    ...
    print("CT shape:", (volume.shape[2], volume.shape[1], volume.shape[0])) # in (nx, ny, nz) order
    ...
    if rtdose_path:
        ...
        print("Dose shape:", (rtdose.shape[2], rtdose.shape[1], rtdose.shape[0])) # (nx, ny, nz)
        ...
        dose_spacing = (dx, dy, dz)  # note the order (dx, dy, dz)
    ...
    return volume, spacing, origin, image_direction, rtstruct, rtplan, rtdose