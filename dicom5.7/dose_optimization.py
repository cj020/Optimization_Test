...
def main():
    ...
    # ------------------------------------------------------------------
    # 1. Load all DICOM data (CT or US, RTSTRUCT, RTPLAN, RTDOSE)
    # ------------------------------------------------------------------
    volume, spacing, origin, direction, rtstruct, rtplan, rtdose, dose_ds, modality = load_dicom(folder)

    print(f"\n{modality} Volume shape: {volume.shape}")
    print(f"{modality} Spacing (dx, dy, dz): {spacing} mm")
    print(f"{modality} Origin  (x0, y0, z0): {origin} mm")
