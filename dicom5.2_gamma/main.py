...
from dose_contribution import dose_contribution, gamma_index_3d
...
def main():
    ...
    # ------------------------------------------------------------------
    # 6. Gamma analysis: resample TG-43 dose onto RTDOSE grid, then compare
    # ------------------------------------------------------------------
    
    if rtdose is not None and dose_ds is not None:
        print(f"\n{'='*70}")
        print("GAMMA ANALYSIS (TG-43 vs RTDOSE)")
        print(f"{'='*70}")

        # --- RTDOSE grid geometry ---
        dose_origin = np.array(dose_ds.ImagePositionPatient, dtype=float)
        dy_dose, dx_dose = [float(v) for v in dose_ds.PixelSpacing]
        z_offsets = np.array(dose_ds.GridFrameOffsetVector, dtype=float)
        dz_dose = float(z_offsets[1] - z_offsets[0]) if len(z_offsets) > 1 else 1.0
        dose_spacing = (dx_dose, dy_dose, dz_dose)

        nz_d, ny_d, nx_d = rtdose.shape
        dose_x = dose_origin[0] + np.arange(nx_d) * dx_dose
        dose_y = dose_origin[1] + np.arange(ny_d) * dy_dose
        dose_z = dose_origin[2] + z_offsets

        print(f"  RTDOSE grid  : {rtdose.shape}  spacing ({dx_dose}, {dy_dose}, {dz_dose}) mm")
        print(f"  RTDOSE origin: {dose_origin}")
        print(f"  CT grid      : {ct_shape}  spacing {tuple(ct_spacing)} mm")

        # --- Resample TG-43 dose (CT grid) onto the RTDOSE grid ---
        print("\n  Resampling TG-43 dose from CT grid onto RTDOSE grid ...")
        ...
        # Build query points: every voxel center in the RTDOSE grid
        dz_grid, dy_grid, dx_grid = np.meshgrid(dose_z, dose_y, dose_x, indexing='ij')
        query_pts = np.column_stack([dz_grid.ravel(), dy_grid.ravel(), dx_grid.ravel()])
        dose_calc_on_rtdose = interp(query_pts).reshape(nz_d, ny_d, nx_d).astype(np.float32)

        print(f"  Resampled calc dose: shape {dose_calc_on_rtdose.shape}, "
              f"max {dose_calc_on_rtdose.max():.4f} Gy")

        dose_ref = rtdose.astype(np.float32)
        print(f"  Reference RTDOSE   : max {dose_ref.max():.4f} Gy")

        # --- Detect RTDOSE pixel saturation ---
        sat_mask = None
        ...
        if dtype_max is not None:
            ...
            if n_saturated > 0:
                ...
                dose_calc_on_rtdose = np.minimum(dose_calc_on_rtdose, dose_cap)

        if np.max(dose_ref) > 0:
            print(f"\n  Max dose ratio (calc/ref): "
                  f"{dose_calc_on_rtdose.max() / dose_ref.max():.3f}")

        # --- Run gamma analysis (3%/3mm) ---
        print()
        gamma, pass_rate = gamma_index_3d(
            dose_vol_1=dose_ref,
            dose_vol_2=dose_calc_on_rtdose,
        ...
        print("\n  No RTDOSE loaded -- skipping gamma analysis.")
        
    # ------------------------------------------------------------------
    # 7. Summary — all data ready for optimization
    # ------------------------------------------------------------------
    print(f"\n{'='*70}")
