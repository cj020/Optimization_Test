...
def mesh_needle_in_ptv(channel_dwells, ptv_mask, ct_origin, ct_spacing,
    ...
    ptv_mask : ndarray (nz, ny, nx), bool
        Combined PTV mask on the CT grid.
    ct_origin : ndarray (3,)   — (x0, y0, z0)
    ct_spacing : ndarray (3,)  — (dx, dy, dz)
    margin : float
    ...
    vox = np.round((probe_pts - ct_origin) / ct_spacing).astype(int)
