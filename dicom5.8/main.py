...
def main():
    ...
    # Native TRUS is typically ~0.2 mm in-plane. TG-43 / IPSA run on a 1 mm
    # calculation grid, matching typical CT voxel size.
    if modality == "US":
        print("\n  US native grid is finer than 1 mm; downsampling for dose calculation.")
        volume, spacing, origin = resample_volume_to_spacing(
            volume, spacing, origin, target_mm=1.0
        )
    ...
    print(f"  CTV ({len(ctv_names)})          : {ctv_names}")
    ...
    for name in mesh_target_names:
        if name in masks:
            ptv_mask |= masks[name]
    print(f"\n  Combined needle-mesh target mask "
          f"({'CTV' if ctv_names else 'PTV'}): {int(ptv_mask.sum())} voxels")
    ...
    J, breakdown = cost_function(dose_gy, masks, ctv_names, oar_names)
    print_cost_report(J, breakdown)

    # ------------------------------------------------------------------
    # 7. IPSA dose optimisation (500 iterations)
    # ------------------------------------------------------------------
    ...
    J_opt, breakdown_opt = cost_function(opt_dose_gy, masks, ctv_names, oar_names)
