...
def ipsa_optimize(dwell_positions, dwell_directions, current_times,
                  current_dose_gy, masks, ptv_names, oar_names,
    ...
    evaluated on the full 3-D grid using the PTV/OAR masks.
    ...
    masks             : dict[str, bool array] — structure masks on CT grid
    ptv_names, oar_names : lists of structure names
    volume            : CT volume array (used by dose_contribution for shape)
    spacing, origin   : CT grid geometry
    ...
    # ---- initial cost ----
    J, _ = cost_function(dose_gy, masks, ptv_names, oar_names, rx_params)
    ...
    for iteration in range(n_iter):
        ...
        J_new, _ = cost_function(proposed_dose, masks, ptv_names, oar_names, rx_params)
        dJ = J_new - J
...
def run_ipsa(dwell_positions, dwell_directions, dwell_times,
    ...
    optimized_times, history = ipsa_optimize(
        ...
        initial_dose_gy, masks, ptv_names, oar_names,
        volume, spacing, origin, L, S_k, Lambda,
