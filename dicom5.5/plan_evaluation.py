...
def evaluate_plan(dose_gy, masks, ptv_names, oar_names, spacing_mm, D_p=None):
    ...
    -------
    report : dict
        Keys are structure roles ("ptv", "urethra", ...).  Each value has
        structure name, metrics, and a list of criterion check results.
    """
    if D_p is None:
    ...
    for role, names in role_names.items():
        ...
        # Union mask when multiple structures share a role (e.g. several PTVs)
        combined = None
        for name in names:
...
def print_evaluation_report(report, D_p=None):
    ...
    role_order = ("ptv", "urethra", "bladder", "rectum")

    for role in role_order:
        info = report.get(role)
        if info is None:
            continue
        title = role.upper() if role != "ptv" else "PTV"
        print(f"\n  --- {title} ---")
        if info.get("missing"):
